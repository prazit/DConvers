...
import com.clevel.dconvers.output.SQLOutput;
...
public class Converter extends AppBase {
    ...
    private Map<Class, Output> outputMap;
    ...
    public boolean print(boolean printSourceTable, boolean printTargetTable, boolean printMappingTable) {
        ...
        for (Target target : sortedTarget) {
            ...
            if (targetConfig.getOutputConfig().isSqlInsert()) {
            ...
            if (targetConfig.getOutputConfig().isSqlCreate()) {
            ...
            if (printTargetTable) {
                ...
                outputFile = targetConfig.getOutputConfig().getSqlOutput();
                headPrint = "--\n"
                ...
                if (targetConfig.getOutputConfig().isMarkdown()) {
                    headPrint = headPrint.replaceAll("--", ">").replaceAll("\n", "  \n");
                    outputFile = targetConfig.getOutputConfig().getMarkdownOutput();
                    success = success && printDataTableToMarkdown(headPrint, dataTable, outputFile, charset);
                }

                if (targetConfig.getOutputConfig().isPdf()) {
                    headPrint = headPrint.replaceAll("--", ">").replaceAll("\n", "  \n");
                    outputFile = targetConfig.getOutputConfig().getPdfOutput();
                    success = success && printDataTableToPDF(headPrint, dataTable, outputFile, targetConfig.getOutputConfig().getPdfJRXML());
                }
            }
        ...
        for (Source source : sortedSource) {
            ...
            if (sourceConfig.getOutputConfig().isSqlInsert()) {
                sqlInsertFormatter = sqlInsert;
            } else {
                sqlInsertFormatter = null;
            }
            if (sourceConfig.getOutputConfig().isSqlCreate()) {
                sqlCreateFormatter = sqlCreate;
            } else {
                sqlCreateFormatter = null;
            }
            ...
            outputFile = outputSourcePath + "V" + sourceFileNumber.getValue() + "__" + source.getSourceConfig().getOutputConfig().getSqlOutput();
            headPrint = "--\n"
                    + "-- Generated by dconvers at " + nowString + ".\n"
            ...
            if (sourceConfig.getOutputConfig().isMarkdown()) {
                headPrint = headPrint.replaceAll("--", ">").replaceAll("\n", "  \n");
                outputFile = sourceConfig.getOutputConfig().getMarkdownOutput();
                success = success && printDataTableToMarkdown(headPrint, dataTable, outputFile, charset);
            }

            if (sourceConfig.getOutputConfig().isPdf()) {
                headPrint = headPrint.replaceAll("--", ">").replaceAll("\n", "  \n");
                outputFile = sourceConfig.getOutputConfig().getPdfOutput();
                success = success && printDataTableToPDF(headPrint, dataTable, outputFile, sourceConfig.getOutputConfig().getPdfJRXML());
            }
        }
    ...
    private boolean printDataTableToPDF(String headPrint, DataTable dataTable, String outputFile, Object jrxmlFile) {
        String tableName = dataTable.getTableName();

        PDFTableFormatter pdfTableFormatter = new PDFTableFormatter(outputFile, jrxmlFile);
    ...
    public List<Output> getOutputList(OutputConfig outputConfig) {
        List<Output> outputList = new ArrayList<>();
        Output output;

        if (outputConfig.isSql()) {
            output = outputMap.get(SQLOutput.class);
            if (output == null) {
                output = new SQLOutput(application, "SQL Output");
                outputMap.put(SQLOutput.class, output);
            }
            outputList.add(output);
        }

        if (outputConfig.isMarkdown()) {
            // TODO Now: create MarkdownOutput and add into the list
        }

        if (outputConfig.isPdf()) {
            // TODO Now: create PDFOutput and add into the list
        }

        if (outputConfig.isTxt()) {
            // TODO Now: create TXTOutput and add into the list
        }

        if (outputConfig.isCsv()) {
            // TODO Now: create CSVOutput and add into the list
        }
